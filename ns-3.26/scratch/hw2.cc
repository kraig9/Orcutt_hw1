...
static Gnuplot
TestDeterministic (Ptr<hw2PropModel> model)
{
...

  {
    ...
    for (double distance = 200; distance < 400; distance += 50)
      {
...
{
  double lossExp = 1;
  std::string variable = "ns3::NormalRandomVariable[Mean=0|Variance=4]";

  CommandLine cmd;
  cmd.AddValue("lossExp", "Loss exponent lossExp", lossExp);
  cmd.AddValue("Variable","Variable",variable);
  cmd.Parse (argc, argv);
  
    GnuplotCollection gnuplots ("output.pdf");
  {
    Ptr<hw2PropModel> log = CreateObject<hw2PropModel> ();
    log->SetAttribute ("Exponent", DoubleValue(lossExp));
	log->SetAttribute ("Variable", StringValue(variable));
	
    Gnuplot plot = TestDeterministic (log);
    plot.SetTitle ("ns3::hw2PropModel ");
